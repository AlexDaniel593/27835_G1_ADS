...
export const productService = {
  ...
  // RF-07: Obtener productos eliminados
  getDeletedProducts: async () => {
    const response = await apiClient.get(`${API_ENDPOINTS.PRODUCTS.BASE}/deleted`);
    return response.data;
  },

  // RF-07: Restaurar producto
  restoreProduct: async (id) => {
    const response = await apiClient.put(`${API_ENDPOINTS.PRODUCTS.BY_ID(id)}/restore`);
    return response.data;
  },
};
