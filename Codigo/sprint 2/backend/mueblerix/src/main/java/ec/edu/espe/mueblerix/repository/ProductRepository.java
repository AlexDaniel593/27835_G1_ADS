...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    // Métodos para funcionalidad de restauración de productos (RF-07)
    @Query("SELECT p FROM Product p WHERE p.isDeleted = true ORDER BY p.deletedAt DESC")
    List<Product> findDeletedProducts();

    @Query("SELECT COUNT(p) > 0 FROM Product p WHERE LOWER(p.name) = LOWER(:name) AND p.isDeleted = false AND p.isActive = true")
    boolean existsActiveProductWithName(@Param("name") String name);

    Optional<Product> findByIdAndIsDeletedTrue(Long id);

    // Para gestión de ofertas (RF-08)
